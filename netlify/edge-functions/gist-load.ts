const handler = async (request: Request) => {
  console.log('Gist Load Edge Function 호출됨');
  
  // CORS 헤더 설정
  const corsHeaders = {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, Authorization',
  };

  // Preflight 요청 처리
  if (request.method === 'OPTIONS') {
    return new Response(null, {
      status: 200,
      headers: corsHeaders,
    });
  }

  // GET 요청만 허용
  if (request.method !== 'GET') {
    return new Response(
      JSON.stringify({ error: 'Method not allowed' }),
      {
        status: 405,
        headers: {
          'Content-Type': 'application/json',
          ...corsHeaders,
        },
      }
    );
  }

  try {
    const url = new URL(request.url);
    const shareUrl = url.searchParams.get('url');

    if (!shareUrl) {
      return new Response(
        JSON.stringify({ error: '공유 URL이 필요합니다.' }),
        {
          status: 400,
          headers: {
            'Content-Type': 'application/json',
            ...corsHeaders,
          },
        }
      );
    }

    console.log('공유 URL 로드 시작:', shareUrl);

    let rawUrl = shareUrl;
    
    // dpaste.com URL 처리 (우선순위)
    if (shareUrl.includes('dpaste.com')) {
      rawUrl = shareUrl.endsWith('.txt') ? shareUrl : `${shareUrl}.txt`;
    }
    // JSONBin URL 처리
    else if (shareUrl.includes('jsonbin.io')) {
      const binId = shareUrl.split('/').pop();
      if (!binId) {
        return new Response(
          JSON.stringify({ error: '올바르지 않은 JSONBin URL입니다.' }),
          {
            status: 400,
            headers: {
              'Content-Type': 'application/json',
              ...corsHeaders,
            },
          }
        );
      }
      rawUrl = `https://api.jsonbin.io/v3/b/${binId}/latest`;
    }
    // JSONStorage.net URL 처리
    else if (shareUrl.includes('jsonstorage.net')) {
      rawUrl = shareUrl; // JSONStorage는 직접 액세스 가능
    }
    // JSONStore URL 처리
    else if (shareUrl.includes('jsonstore.io')) {
      rawUrl = shareUrl; // JSONStore는 직접 액세스 가능
    }
    // GitHub Gist URL 처리
    else if (shareUrl.includes('gist.github.com')) {
      const gistId = shareUrl.split('/').pop();
      if (!gistId) {
        return new Response(
          JSON.stringify({ error: '올바르지 않은 Gist URL입니다.' }),
          {
            status: 400,
            headers: {
              'Content-Type': 'application/json',
              ...corsHeaders,
            },
          }
        );
      }
      
      // Gist 정보를 가져와서 파일의 raw URL을 찾음
      const gistResponse = await fetch(`https://api.github.com/gists/${gistId}`, {
        headers: {
          'Accept': 'application/vnd.github.v3+json',
          'User-Agent': 'MyKallaxis-Collection-App',
        },
      });
      
      if (!gistResponse.ok) {
        return new Response(
          JSON.stringify({ error: 'Gist를 찾을 수 없습니다.' }),
          {
            status: 404,
            headers: {
              'Content-Type': 'application/json',
              ...corsHeaders,
            },
          }
        );
      }
      
      const gistData = await gistResponse.json();
      const firstFile = Object.values(gistData.files)[0] as { raw_url: string };
      rawUrl = firstFile.raw_url;
    }

    console.log('Raw URL 생성:', rawUrl);

    // Raw 데이터 가져오기
    const headers: Record<string, string> = {
      'User-Agent': 'MyKallaxis-Collection-App',
    };

    // JSONBin API의 경우만 API 키 추가 (환경 변수에서 가져올 수 없으므로 생략)
    // Edge Functions에서는 Netlify 환경 변수 접근이 제한적

    const response = await fetch(rawUrl, {
      headers,
    });
    
    console.log('데이터 가져오기 응답 상태:', response.status);
    
    if (!response.ok) {
      return new Response(
        JSON.stringify({ error: `데이터를 불러올 수 없습니다: ${response.statusText}` }),
        {
          status: response.status,
          headers: {
            'Content-Type': 'application/json',
            ...corsHeaders,
          },
        }
      );
    }

    // dpaste의 경우 텍스트를 JSON으로 파싱
    let data;
    if (rawUrl.includes('dpaste.com')) {
      const textData = await response.text();
      data = JSON.parse(textData);
    } else {
      data = await response.json();
    }
    
    // JSONBin에서 온 데이터는 record 속성에 담겨있을 수 있음
    const collectionData = data.record || data;
    
    console.log('컬렉션 데이터 로드 완료:', {
      username: collectionData._metadata?.username,
      collectionName: collectionData._metadata?.collectionName,
      albumCount: collectionData.albums?.length
    });
    
    return new Response(JSON.stringify(collectionData), {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
        ...corsHeaders,
      },
    });

  } catch (error) {
    console.error('Collection load error:', error);
    return new Response(
      JSON.stringify({ 
        error: '컬렉션 로드 중 오류가 발생했습니다.',
        details: error instanceof Error ? error.message : 'Unknown error'
      }),
      {
        status: 500,
        headers: {
          'Content-Type': 'application/json',
          ...corsHeaders,
        },
      }
    );
  }
};

export default handler;
